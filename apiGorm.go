...
func returnAllBookings(w http.ResponseWriter, r *http.Request) {
	bookings := []Booking{}
	db.Find(&bookings)
	fmt.Println("Endpoint Hit: returnAllBooking")
...
func handleRequests() {
	...
	myRouter.HandleFunc("/booking/{id}", returnSinbleBookingdos).Methods("GET")
...
func main() {
	...
	db, err = gorm.Open("mysql", "root:password@tcp(127.0.0.1:3306)/apigo?charset=utf8&parseTime=True")
